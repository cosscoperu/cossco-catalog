...
export default function CartDrawer({cart,onRemove,onQty}){
  ...
  return (
  <div className="sticky bottom-4 right-4 z-30 md:fixed md:bottom-4 md:right-4">
    <div className="w-72 rounded-2xl border border-plateado/40 bg-white shadow-suave">
      ...
      <div className="max-h-64 space-y-3 overflow-auto p-3">
        {cart.length === 0 ? (
          <div className="text-sm text-plateado">Aún no has agregado productos.</div>
        ) : (
          cart.map((it, i) => (
            <div key={i} className="flex items-center justify-between gap-2">
              <div className="flex-1">
                <div className="text-sm font-medium">{it.title}</div>
                <div className="text-xs text-plateado">{it.price}</div>
              </div>
      ...
      <div className="flex items-center justify-between border-t border-plateado/30 p-3">
        ...
        <div className="text-base font-semibold">S/ {total.toFixed(2)}</div>
      </div>

      <div className="p-3">
        <a href={wa} target="_blank" rel="noreferrer" className="block w-full rounded-xl bg-dorado px-3 py-2 text-center font-semibold text-grafito hover:bg-doradoHover">
